...
plugin "rs_azure_sqlj" do
  ...
  type "sql_server" do
    href_templates "{{type=='Microsoft.Sql/servers' && join('?',[id,'api-version=2019-06-01-preview']) || null}}"
    ...
    action "create" do
      type "sql_server"
      path "/subscriptions/$subscription_id/resourceGroups/$resource_group/providers/Microsoft.Sql/servers/$name?api-version=2019-06-01-preview"
      verb "PUT"
    end
    ...
    link "databases" do
      path "$id/databases?api-version=2017-10-01-preview"
      type "databases"
      output_path "value[*]"
    end
    ...
    link "elastic_pools" do
      path "$id/elasticPools?api-version=2017-10-01-preview"
      type "elastic_pool"
      output_path "value[*]"
    end
  end
  ...
  type "databases" do
    href_templates "{{type=='Microsoft.Sql/servers/databases' && join('?',[id,'api-version=2017-10-01-preview']) || null}}","{{value[0].type=='Microsoft.Sql/servers/databases' && map(&join('?',[id,'api-version=2017-10-01-preview']),value) || null}}"
    ...
    action "create" do
      type "databases"
      path "/subscriptions/$subscription_id/resourceGroups/$resource_group/providers/Microsoft.Sql/servers/$server_name/databases/$name?api-version=2017-10-01-preview"
      verb "PUT"
    end
 ...
 type "transparent_data_encryption" do
  ...
  type "elastic_pool" do
    href_templates "{{type=='Microsoft.Sql/servers/elasticPools' && join('?',[id,'api-version=2017-10-01-preview']) || null}}","{{value[0].type=='Microsoft.Sql/servers/elasticPools' && map(&join('?',[id,'api-version=2014-04-01']),value) || null}}"
    ...
    action "create" do
      path "/subscriptions/$subscription_id/resourceGroups/$resource_group/providers/Microsoft.Sql/servers/$server_name/elasticPools/$name?api-version=2017-10-01-preview"
      verb "PUT"
    end
    ...
    action "get_database" do
      path "/subscriptions/$subscription_id/resourceGroups/$resource_group/providers/Microsoft.Sql/servers/$server_name/elasticPools/$name/databases/$database_name?api-version=2017-10-01-preview"
      verb "GET"
  ...
  type "security_policy" do
    href_templates "{{type=='Microsoft.Sql/servers/databases/securityAlertPolicies' && join('?',[id,'api-version=2019-06-01-preview']) || null}}"
    ...
    action "create" do
      path "/subscriptions/$subscription_id/resourceGroups/$resource_group/providers/Microsoft.Sql/servers/$server_name/databases/$database_name/securityAlertPolicies/$name?api-version=2019-06-01-preview"
      verb "PUT"
    end
  ...
  type "auditing_policy" do
    href_templates "{{type=='Microsoft.Sql/servers/databases/auditingSettings' && join('?',[id,'api-version=2017-03-01-preview']) || null}}"
    ...
    action "create" do
      path "/subscriptions/$subscription_id/resourceGroups/$resource_group/providers/Microsoft.Sql/servers/$server_name/databases/$database_name/auditingSettings/$name?api-version=2017-03-01-preview"
      verb "PUT"
    end
...
resource_pool "rs_azure_sqlj" do
    plugin $rs_azure_sqlj
    parameter_values do
      subscription_id $subscription_id
    end

    auth "azure_auth", type: "oauth2" do
      token_url "https://login.microsoftonline.com/80a3bc8a-6d35-41fc-88a3-fbefdb06743f/oauth2/token"
      grant type: "client_credentials" do
        client_id cred("AZURE_APPLICATION_ID_R")    # Application access key
        client_secret cred("AZURE_APPLICATION_KEY_R")    #Secret key
...
define provision_resource(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    $type = $object["type"]
    call sys_logs.set_task_target(@@deployment)
    call sys_logs.summary(join(["Provision ", $type]))
    call sys_logs.detail($object)
    call start_debugging()
    @operation = rs_azure_sqlj.$type.create($fields)
    call sys_logs.detail(to_object(@operation))
    @resource = @operation.show()
    $status = @resource.state
    sub on_error: skip, timeout: 60m do
      while $status != "Ready" do
        $status = @resource.state
        call sys_logs.detail(join(["Status: ", $status]))
        sleep(10)
      end
    end
    call sys_logs.detail(to_object(@resource))
    call stop_debugging()
  end
end
...
define provision_elastic_pool(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    $server_name = $fields["server_name"]
    $resource_group = $fields["resource_group"]
    call sys_logs.detail("entering check for elastic pool created")
    sub on_error: retry, timeout: 60m do
      call sys_logs.detail("sleeping 10")
      sleep(10)
      @new_resource = @operation.show(name: $name, server_name: $server_name, resource_group: $resource_group )
